...
export const GEOGRAPHIC_DIRECTORY: Record<string, GeoState[]> = {
  US: [
...
export function normalizeCountryCode(countryCode: string): string {
  if (!countryCode || countryCode === 'All' || countryCode === 'ALL') return 'US';
  const c = countryCode.toUpperCase();
  if (c === 'UK') return 'GB';
  return c;
}

export function getAllStatesForCountry(countryCode: string): GeoState[] {
  const code = normalizeCountryCode(countryCode);
  return GEOGRAPHIC_DIRECTORY[code] || GEOGRAPHIC_DIRECTORY['US'] || [];
}

export function getAllCitiesForCountryAndState(countryCode: string, stateNameOrCode?: string): string[] {
  const states = getAllStatesForCountry(countryCode);
  if (!stateNameOrCode || stateNameOrCode === 'All' || stateNameOrCode === 'ALL') {
